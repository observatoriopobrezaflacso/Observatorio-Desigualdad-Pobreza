...
foreach y of numlist 1991(2)2023 2024 {
    di "**********`y'***********"		
	...
	capture describe adec using "$procesado/ingresos_pc/ing_perca_`y'_urb_precios2000.dta"
	if !_rc local adec_var adec
	else local adec_var
	
	capture describe area using "$procesado/ingresos_pc/ing_perca_`y'_urb_precios2000.dta"
	if !_rc local area_var area
	else local area_Var

		
	use w t `adec_var' `condact_var' `instr_var' `area_var' anio fexp using  "$procesado/ingresos_pc/ing_perca_`y'_urb_precios2000.dta", clear
...
use condact nivin* adec anio fexp w t using "$procesado/empleo_adecuado/empleo_1991.dta", clear

foreach y of numlist 1993(2)2023 2024 {
    di "**********`y'***********"		
    ...
    append using "$procesado/empleo_adecuado/empleo_`y'.dta", keep(condact* nivins anio fexp `adec_var' `area_var' w t)
...
if `anio' == 2001                   replace universitario = 1 if `var' >= 6 & `var' != .
else if inrange(`anio', 1990, 1999) replace universitario = 1 if `var' == 5 & `var' != .
...
tabstat universitario [w = fexp], by(anio)


tab nivins_2024 [iw = fexp]
...
replace emp_adec = adec if inrange(anio , 1991, 2006) | anio == 2015 
}
...
foreach y of numlist 2001 2003 2005 2015 {
	...
	use `condact_var' `instr_var' anio fexp using  "$procesado/ingresos_pc/ing_perca_`y'_urb_precios2000.dta", clear
