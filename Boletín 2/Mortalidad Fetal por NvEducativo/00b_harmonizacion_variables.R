...
#' @param anio Año del dataset (opcional, para casos especiales como 2017)
#' @return Data frame con nombres de variables estandarizados
#'
harmonizar_variables_defunciones <- function(df, anio = NULL) {  # ← CAMBIO 1: Agregar parámetro anio
  ...
  # 🔴 CAMBIO 2: Caso especial año 2017 (NUEVO BLOQUE)
  if (!is.null(anio) && anio == 2017) {
    cat("  ⚠️ AÑO 2017 DETECTADO: Usando variable 'causa' en lugar de 'causa103'\n")
    
    # Guardar causa103 original (todo 999) como referencia
    if ("causa103" %in% names(df)) {
      df$causa103_original <- df$causa103
      cat("  📌 causa103 original guardada como 'causa103_original'\n")
    }
    
    # Reemplazar causa103 con causa (códigos CIE-10 reales)
    if ("causa" %in% names(df)) {
      df$causa103 <- df$causa
      cat("  ✅ causa103 reemplazada con códigos CIE-10 de variable 'causa'\n")
    } else {
      warning("  ❌ AÑO 2017: No se encontró variable 'causa'")
    }
  }
  # 🔴 FIN CAMBIO 2
...
cat("  - harmonizar_variables_defunciones(anio = NULL)\n")  # ← Actualizado
