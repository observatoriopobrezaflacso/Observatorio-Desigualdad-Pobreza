...
# Definir espejo de CRAN por defecto
...
# Lista de librerías que usas en el Observatorio
librerias <- c(
  "dplyr", "haven", "ggplot2", "tidyr", "scales",
  "forcats", "DBI", "duckdb", "patchwork", "tibble"
)

# Ejecutar instalación y carga
lapply(librerias, instalar_si_falta)
...
poblacion_adulta_superior <- data.frame(
  superior = c(643564, 1087081, 2205068),
...
poblacion_joven_secundaria <- data.frame(
  secundaria_completa = c(685649, 1673093, 2544173),
...
extrapolacion_poblacion_joven_secundaria <-
  ...
  mutate(
    secundaria_completa = case_when(
      anio == 2001 ~ poblacion_joven_secundaria$secundaria_completa[1],
    ...
    secundaria_incompleta = case_when(
      anio == 2001 ~ poblacion_joven_secundaria$secundaria_incompleta[1],
...
pobreza_raw <- pobreza_raw %>%
  mutate(across(
    c(total_personas, no_pobres_total, pobres_total),
    ~ as.numeric(gsub(",", "", .))
  ))
