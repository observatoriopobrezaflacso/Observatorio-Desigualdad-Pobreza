...
* Define paths
global user_root "/Users/vero/Library/CloudStorage/GoogleDrive-observatorio.pobreza@flacso.edu.ec"
global dir      "$user_root/Mi unidad/Bases"
global output_dir    "$user_root/Mi unidad/Boletín 2/Resultados/Mortalidad/"
global graf_datos_dir "$user_root/Mi unidad/Boletín 2/Resultados/Mortalidad/Datos"
global carpeta_procesadas_edg "$dir/Defunciones/Procesadas/defunciones infantiles"
global carpeta_procesadas_env "$dir/ENV/Procesadas/Infantiles"

global carpeta_edg "$dir/Defunciones/Raw"
global carpeta_env "$dir/ENV/Raw"
...
forvalues y = 2013/2024 {
	...
	di "$carpeta_edg\EDG_`y'.sav"
	
    import spss using "$carpeta_edg/EDG_`y'.sav", clear
    ...
    * Recodificar
    ...
    append using "$carpeta_procesadas/base_muertes"
    save "$carpeta_procesadas/base_muertes", replace
}
...
forvalues y = 2013/2024 {
    ...
    import spss using "$carpeta_env/ENV_`y'.sav", clear
    ...
    * Recodificar
    ...
    if (`y' != 2013) append using "$carpeta_procesadas_env/base_nacimientos"
    save "$carpeta_procesadas_env/base_nacimientos", replace
}

* ── MERGE Y CÁLCULO TMI ──────────────────────────────────────────────────────

use "$carpeta_procesadas_edg/base_muertes", clear
merge 1:1 anio using "$carpeta_procesadas_env/base_nacimientos"
keep if _merge == 3
drop _merge
...
twoway ///
    ...
    ylabel(0(5)20, grid glcolor(gs14) glwidth(vthin) labsize(small)) ///
...
export excel using "$graf_datos_dir/mortalidad_infantil_etnia.xlsx", firstrow(var) replace
	
graph export "$output_dir/TMI_etnia_2013_2024.png", ///
