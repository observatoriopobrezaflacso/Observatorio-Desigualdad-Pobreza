...
global dir_work_data "$basedir/Bases/Fake/Work Data"
global dir_out  "$basedir/Resultados"

capture mkdir "$dir_out"
capture mkdir "$dir_work_data"
...
foreach yr of local years_both {
    ...
    * ------------------------------------------------------------------
    * 5.4  Guardar base de trabajo  (Para el futuro)
    * ------------------------------------------------------------------
	
	save "$dir_work_data/work_`yr'.dta", replace
