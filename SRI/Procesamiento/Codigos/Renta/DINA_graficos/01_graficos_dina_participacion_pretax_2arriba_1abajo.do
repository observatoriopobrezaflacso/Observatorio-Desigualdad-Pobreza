/*******************************************************************************
* Grafico DINA 1 (layout 2+1): participacion PreTaxHHI por parejas
*   Fila superior: Top 0.1% vs 35% | Top 1% vs 50%
*   Fila inferior: Top 10% vs clase media centrado; fxsize 25-50-25 (misma anchura que cada panel superior)
*   Eje Y: ticks 0, paso, ... hasta techo >= max del panel; export ancho amplio
*
* Misma fuente y estilo que 01_graficos_dina_participacion_pretax_3paneles.do
* Salidas: participacion_ingreso_pretax_2010_2024_2arriba_1abajo.{png,pdf,gph}
*******************************************************************************/
...
* ---------------------------------------------------------------------------
* Tres graficos en memoria; fila superior 2 cols, fila inferior 1 grafico
...
* Espacios en blanco para centrar el panel inferior (misma fraccion de ancho que arriba)
foreach s in spL spR {
    quietly twoway scatteri 0 0, msymbol(none) msize(zero) ///
        ...
        fxsize(25) ///
        name(dina_pretax_`s', replace)
}

graph combine dina_pretax_p1 dina_pretax_p2, cols(2) ///
    graphregion(color(white)) ///
    imargin(tiny) ///
    name(dina_pretax_toprow, replace)

graph combine dina_pretax_spL dina_pretax_p3 dina_pretax_spR, cols(3) ///
    graphregion(color(white)) ///
    imargin(tiny) ///
    name(dina_pretax_botrow, replace)

graph combine dina_pretax_toprow dina_pretax_botrow, rows(2) ///
    title("Participación en el ingreso nacional", size(medium)) ///
    subtitle("PreTaxHHI", size(small)) ///
    graphregion(color(white)) ///
    imargin(tiny) ///
    name(dina_pretax_211_comb, replace)

capture noisily graph export "$out_dir/participacion_ingreso_pretax_2010_2024_2arriba_1abajo.png", ///
    replace width(5200) height(3400)
graph export "$out_dir/participacion_ingreso_pretax_2010_2024_2arriba_1abajo.pdf", replace
graph save "$out_dir/participacion_ingreso_pretax_2010_2024_2arriba_1abajo.gph", replace

graph drop dina_pretax_p1 dina_pretax_p2 dina_pretax_p3 dina_pretax_spL dina_pretax_spR ///
    dina_pretax_toprow dina_pretax_botrow dina_pretax_211_comb
