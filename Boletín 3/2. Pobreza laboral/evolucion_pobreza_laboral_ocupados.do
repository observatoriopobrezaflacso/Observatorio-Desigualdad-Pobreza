...
foreach y of numlist 2001(2)2007 2008(1)2025  {
	...
	if `first_file' {
		...
		cap confirm variable CONDACT
		if !_rc rename CONDACTN*, lower
		
		keep `append_vars' `svyvars_actual' condact*
...
replace condactn = condact if inrange(anio, 2019, 2025)
drop if p24 == 999
...
gen ocupado = !inlist(condactn, 0, 7, 8, 9) if anio >= 2007
replace ocupado = !inrange(condact, 5, 8) if anio < 2007
