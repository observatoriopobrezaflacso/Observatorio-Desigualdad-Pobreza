...
preserve 
    keep if area == 1
    collapse (mean) informal2 [iw = fexp], by(anio sexo)
    * Generar líneas separadas para hombres y mujeres
    twoway (line informal2 anio if sexo == 1) ///
           (line informal2 anio if sexo == 2), ///
           legend(label(1 "Hombre") label(2 "Mujer")) ///
           xlabel(, angle(45)) ///
           ylabel(, angle(0)) ///
           yscale(range(0 1)) ylabel(#5, format(%9.2f)) ///
           graphregion(color(white)) bgcolor(white) ///
		   name(informal2_sexo_urb, replace)
    graph export "$out_results/informal2_sexo_urb.png", replace
restore	   
...
preserve 
    ...
    twoway (line informal2 anio if sexo == 1) ///
           ...
           yscale(range(0 1)) ylabel(#5, format(%9.2f)) ///
           graphregion(color(white)) bgcolor(white) ///
           name(informal2_sexo_nac, replace)
    graph export "$out_results/informal2_sexo_nac.png", replace
restore	 
...
preserve
    ...
    * Calcular promedio de informalidad por provincia
    collapse (mean) informal2 [iw = fexp], by(provincia)
    
    * Ordenar por tasa de informalidad (de mayor a menor)
    gsort -informal2
    
    * Generar gráfico de barras horizontales
    graph hbar informal2, over(provincia, sort(1) descending label(labsize(small))) ///
        ylabel(0(0.2)1, format(%9.2f)) ///
        ytitle("Informalidad") ///
        bar(1, color(navy)) ///
        graphregion(color(white)) bgcolor(white) ///
        title("Informalidad por provincia - 2024", size(medium)) ///
        name(informal2_provincia_2024, replace)
    graph export "$out_results/informal2_provincia_2024.png", replace
restore
...
preserve 
    keep if anio >= 2000
    collapse (mean) informal2 [iw = fexp], by(anio age_cat)
    twoway (line informal2 anio if age_cat == 1) ///
           (line informal2 anio if age_cat == 2) ///
           (line informal2 anio if age_cat == 3), ///
           legend(label(1 "18-29") label(2 "30-64") label(3 "65+")) ///
           xlabel(, angle(45)) ///
           ylabel(, angle(0)) ///
           yscale(range(0 1)) ylabel(#5, format(%9.2f)) ///
           graphregion(color(white)) bgcolor(white) ///
           name(informal2_edad_nac, replace)
    graph export "$out_results/informal2_edad_nac.png", replace
restore	 
...
preserve 
    ...
    twoway (line informal2 anio if age_cat == 1) ///
           ...
           yscale(range(0 1)) ylabel(#5, format(%9.2f)) ///
           graphregion(color(white)) bgcolor(white) ///
           name(informal2_edad_urb, replace)
    graph export "$out_results/informal2_edad_urb.png", replace
restore	 
...
preserve 
    keep if anio >= 2000
    collapse (mean) informal2 [iw = fexp], by(anio etnia_arm)
    twoway (line informal2 anio if etnia_arm == 1) ///
           (line informal2 anio if etnia_arm == 2) ///
           (line informal2 anio if etnia_arm == 3), ///
           legend(label(1 "Indigena") label(2 "Negro/Afro") ///
		   label(3 "Blanco/Mestizo")) ///
           xlabel(, angle(45)) ///
           ylabel(, angle(0)) ///
           yscale(range(0 1)) ylabel(#5, format(%9.2f)) ///
           graphregion(color(white)) bgcolor(white) ///
           name(informal2_etnia_nac, replace)
    graph export "$out_results/informal2_etnia_nac.png", replace
restore	 
...
preserve 
    ...
    twoway (line informal2 anio if etnia_arm == 1) ///
           ...
           yscale(range(0 1)) ylabel(#5, format(%9.2f)) ///
           graphregion(color(white)) bgcolor(white) ///
           name(informal2_etnia_urb, replace)
    graph export "$out_results/informal2_etnia_urb.png", replace
restore	 
...
preserve 
    keep if anio >= 2000
    collapse (mean) informal2 [iw = fexp], by(anio educ_univ)
    twoway (line informal2 anio if educ_univ == 0) ///
           (line informal2 anio if educ_univ == 1), ///
           legend(label(1 "No universitaria") label(2 "Universitaria")) ///
           xlabel(, angle(45)) ///
           ylabel(, angle(0)) ///
           yscale(range(0 1)) ylabel(#5, format(%9.2f)) ///
           graphregion(color(white)) bgcolor(white) ///
           name(informal2_educ_nac, replace)
    graph export "$out_results/informal2_educ_nac.png", replace
restore	 
...
preserve 
    ...
    twoway (line informal2 anio if educ_univ == 0) ///
           ...
           yscale(range(0 1)) ylabel(#5, format(%9.2f)) ///
           graphregion(color(white)) bgcolor(white) ///
           name(informal2_educ_urb, replace)
    graph export "$out_results/informal2_educ_urb.png", replace
restore
